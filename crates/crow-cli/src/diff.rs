...
pub fn render_plan_diff(source_root: &Path, sandbox_root: &Path, plan: &IntentPlan) {
    ...
    println!();
    println!("  {}", "◆ Plan Diff Summary".bold().with(crossterm::style::Color::AnsiValue(221)));
    ...
    println!("    {}", "(View full diff in .crow/logs/latest.patch)".with(crossterm::style::Color::AnsiValue(240)));
}
...
fn print_file_stat(path: &str, added: usize, removed: usize, op: &str) {
    ...
    let a_str = if added > 0 { 
        format!("+{}", added).with(crossterm::style::Color::AnsiValue(114)).to_string() 
    } else { 
        "  ".to_string() 
    };
    
    let r_str = if removed > 0 { 
        format!("-{}", removed).with(crossterm::style::Color::AnsiValue(203)).to_string() 
    } else { 
        "  ".to_string() 
    };
    ...
    if total > 0 {
        ...
        bar_str.push_str(&"█".repeat(final_added_ticks).with(crossterm::style::Color::AnsiValue(114)).to_string());
        bar_str.push_str(&"█".repeat(removed_ticks).with(crossterm::style::Color::AnsiValue(203)).to_string());
    } else {
        bar_str = "░".repeat(max_ticks).with(crossterm::style::Color::AnsiValue(240)).to_string();
    }
    
    let total_stat = format!("{:>18} {:>18}", a_str, r_str);
    
    println!("    {} |{}| {} [{}]", path_disp.with(crossterm::style::Color::AnsiValue(248)), bar_str, total_stat, op.with(op_color));
}
