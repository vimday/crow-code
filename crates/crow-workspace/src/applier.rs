...
///
/// Uses GitWorkspaceManager for fast, reliable transaction rollbacks.
pub async fn apply_sandbox_to_workspace(
    workspace_root: &std::path::Path,
    plan: &crow_patch::IntentPlan,
) -> Result<(), anyhow::Error> {
    // 1. Snapshot original state using git
    ...
    // 4. Commit and merge
    git_manager.commit_and_merge().await?;
