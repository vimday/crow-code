...
- **Step 17** ✅ Auto Mode Orchestration: Added `/auto <task>` and `/agent`/`/agents` status commands, deterministic `crow-runtime::auto` planning, registry-tracked subagent reservations, orchestration events, and TUI AutoHUD/status feed rendering.
